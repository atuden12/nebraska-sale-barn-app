...
import { fetchCattleSlaughter } from "@/lib/api/usda-nass";
...
export async function GET() {
  try {
    const slaughterData = await fetchCattleSlaughter();
...
// Demo data when API is unavailable - NATIONAL scope
function getDemoSlaughterData(): SlaughterData[] {
  ...
  for (let i = 0; i < 8; i++) {
    const date = new Date();
    date.setDate(date.getDate() - 7 * i - date.getDay());

    const variance = (Math.random() - 0.5) * 20000;
    const currentSlaughter = Math.round(baseSlaughter + variance - i * 5000);
    const prevWeekSlaughter = Math.round(
      currentSlaughter * (1 + (Math.random() - 0.5) * 0.05)
    );
    const prevYearSlaughter = Math.round(
      currentSlaughter * (1 + (Math.random() - 0.5) * 0.08)
    );
    ...
    weeks.push({
      ...
      percentChangeWeek:
        ((currentSlaughter - prevWeekSlaughter) / prevWeekSlaughter) * 100,
      percentChangeYear:
        ((currentSlaughter - prevYearSlaughter) / prevYearSlaughter) * 100,
      region: "National",
    });
  }
